...
(function( $ ) {
	...
	$.as.ui = {
		...
		*/
		...
		/*
			스크롤 이벤트 바인딩
			Use --> $.as.ui.scroll( down_fn, up_fn, interval );
		*/
		scroll: function(down_fn, up_fn, interval){
			var interval = interval || 250,
			      downFunc = down_fn || function(){},
			      upFunc = up_fn || function(){},
			      lastScroll = 0;

			$(window).bind('scroll', function(e){
				var st = $(this).scrollTop();
				window.scrollTimeoutEvent;
				$(window).scroll(function(){
					clearTimeout( window.scrollTimeoutEvent );
					window.scrollTimeoutEvent = setTimeout(function(){
						if( st > lastScroll ) {
							downFunc();
						} else {
							upFunc();
		...
		/*
			마우스 휠 이벤트 바인딩
			Use --> $.as.ui.wheel( down_fn, up_fn, interval );
		*/
		wheel: function(down_fn, up_fn, interval){
			var interval = interval || 250,
			      downFunc = down_fn || function(){},
			      upFunc = up_fn || function(){};

			$('html, body').on('mousewheel DOMMouseScroll', function(e){
				var wheel_direction = e.originalEvent,
					delta = 0;
				if (wheel_direction.detail) {
					delta = wheel_direction.detail * -40;
				}else{
					delta = wheel_direction.wheelDelta;
				};

				window.wheelTimeoutEvent;
				$('html, body').on('mousewheel DOMMouseScroll', function(){
					clearTimeout( window.wheelTimeoutEvent );
					window.wheelTimeoutEvent = setTimeout(function(){
						if( delta < 0 ) {
							downFunc();
						} else {
							upFunc();
